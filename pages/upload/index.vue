<template>
  <nuxt-layout name="upload-layout">
    ...
    <div
      class="w-full h-[calc(100%-60px)] bg-color-bg-creator flex items-center"
      :class="!ui.isUploadPost ? 'justify-center' : 'justify-start'">
      <div v-if="!ui.isUploadPost" class="w-full flex justify-center">
        <input id="file-upload" type="file" class="hidden" accept="video/*" @change="onChange">
        <label for="file-upload" class="w-[90%] bg-white md:p-10 max-w-[1000px]">
          <div class="bg-color-bg-creator" @drop.prevent="drop">
      ...
      <form-upload-post v-if="fileData" :file-data="fileData" />
    </div>
  </nuxt-layout>
</template>
